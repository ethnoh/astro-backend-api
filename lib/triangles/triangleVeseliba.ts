...
// === Core math for Veseliba ===
function calcVeselibaNumbers(dateStr: string) {
  ...
  // внутренняя сумма (день + месяц + год)
  const baseSum = reduce22(dayReduced + monthReduced + yearReduced);

  // (1) Верхняя = день + месяц + год + (день + месяц + год)
  const top = reduce22(dayReduced + monthReduced + yearReduced + baseSum);

  // (2) Нижняя правая = (1) + день
  const bottomRight = reduce22(top + dayReduced);

  // (3) Нижняя левая = (1) + (день + месяц + год)
  const bottomLeft = reduce22(top + baseSum);
...
// === MAIN DRAW ===
export function drawTriangleVeseliba(dateStr = "10.08.1990") {
  const nums = calcVeselibaNumbers(dateStr);
